...
def _explode_column_period_scores(df_to_explode: pd.DataFrame):
    ...
    grouped[
        [
            "sport_event_status.period_scores.regular_period.1.home_score",
            "sport_event_status.period_scores.regular_period.1.away_score",
        ]
    ] = grouped[0].str.split("x", expand=True)
    grouped[
        [
            "sport_event_status.period_scores.regular_period.2.home_score",
            "sport_event_status.period_scores.regular_period.2.away_score",
        ]
    ] = grouped[1].str.split("x", expand=True)
    if 2 in grouped.columns:
        grouped[
            [
                "sport_event_status.period_scores.overtime.home_score",
                "sport_event_status.period_scores.overtime.away_score",
            ]
        ] = grouped[2].str.split("x", expand=True)
    if 3 in grouped.columns:
        grouped[
            [
                "sport_event_status.period_scores.overtime.home_score",
                "sport_event_status.period_scores.overtime.away_score",
...
def format_competitions(response: requests.models.Response):
...
def format_seasons(response: requests.models.Response):
...
def format_season_summary(response: requests.models.Response):
...
def format_season_players_statistics(response: requests.models.Response):
    ...
    season_players_statistics = season_players_statistics.loc[
        season_players_statistics["sport_event_status.match_status"] == "ended"
    ]
    season_players_statistics = _explode_column(
        season_players_statistics,
        "statistics.totals.competitors",
        [
            "sport_event.id",
    ...
    season_players_statistics.columns = [
        col.split(".")[-1] if ".players." in col else ".".join(col.split(".")[-2:])
        for col in season_players_statistics.columns
    ]
    season_players_statistics[
        season_players_statistics.select_dtypes(float).columns
    ] = season_players_statistics.select_dtypes(float).astype("int64")

    return season_players_statistics


def format_season_competitors_statistics(response: requests.models.Response):
    season_competitors_statistics = pd.json_normalize(response.json(), "summaries")

    if season_competitors_statistics.empty:
        return pd.DataFrame({"sport_event.sport_event_context.season.id": [response.url.split("/")[-2]]})

    season_competitors_statistics = season_competitors_statistics.loc[
        season_competitors_statistics["sport_event_status.match_status"] == "ended"
    ]
    season_competitors_statistics = _explode_column(
        season_competitors_statistics,
        "statistics.totals.competitors",
        [
            "sport_event.id",
            "sport_event.sport_event_context.competition.id",
    ...
    season_competitors_statistics.columns = [
        col.split(".")[-1] if col.startswith("statistics") else col.replace("sport_event.sport_event_context.", "")
        for col in season_competitors_statistics.columns
    ]
    season_competitors_statistics[
        season_competitors_statistics.select_dtypes(float).columns
    ] = season_competitors_statistics.select_dtypes(float).astype("int64")

    return season_competitors_statistics


def format_season_referees(response: requests.models.Response):
    ...
    season_referees = season_referees.loc[
        :,
        [
            "sport_event.id",
            "sport_event.sport_event_context.season.id",
...
def format_season_ball_locations(response: requests.models.Response):
    ...
    season_ball_locations = season_ball_locations.loc[
        :,
        [
            "sport_event.id",
            "sport_event.sport_event_context.season.id",
    ...
    season_ball_locations = season_ball_locations.rename(
        columns={"sport_event.sport_event_context.season.id": "season.id"}
    )
...
def format_season_channels(response):
    ...
    season_channels = season_channels.loc[
        :,
        [
            "sport_event.id",
            "sport_event.sport_event_context.season.id",
...
def format_player_profile(response: requests.models.Response):
